...
def load_all_assessment_data(output_dir):
    all_assessments = []
...
def render_demographics(df):
    ...
    with col1:
        ...
        specialty_counts = df.groupby('name')['specialty'].first().value_counts()
        ...
        exp_counts = df.groupby('name')['years_experience'].first().value_counts()
        st.bar_chart(exp_counts)

    with col2:
        st.subheader("Experience with OCTA Imaging")
        octa_counts = df.groupby('name')['octa_experience'].first().value_counts()
        st.bar_chart(octa_counts)
        
        st.subheader("Familiarity with AI")
        ai_counts = df.groupby('name')['ai_familiarity'].first().value_counts()
...
def render_evaluation_page():
    ...
    df = load_all_assessment_data(OUTPUT_DIR)
    
    if df.empty:
        st.error("Could not load any assessment data. Please check the `results` directory.")
        return
